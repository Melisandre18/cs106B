...
bool singleQueueRandomTest(MultiQueue * q, int queueIndex) {
	...
	for (int i=0; i<num; i++) {
		int v = randomInteger(0, 100);
		int dequeue = false;//randomChance(0.8);
		if (dequeue) {
			if (qc.dequeue() != q->dequeue(queueIndex)) {
				return false;
			}
...
bool multiQueueRandomTest() { 
	MultiQueue * q;
	q = new MultiQueue();
	int num = randomInteger(100, 10000);
	for (int i=0; i<num; i++) {
		q->addQueue();
	}
...
void runMultiQueueTests() {
	...
	passed = testEnqueueGrow();
	for (int i=0; i<20; i++) {
		printTestResults(passed, testNum, passedCounter);
	}
	passed = testAddQueueGrow();
	for (int i=0; i<5; i++) {
		printTestResults(passed, testNum, passedCounter);
	}
	//timeTest();
